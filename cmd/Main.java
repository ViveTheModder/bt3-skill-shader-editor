...
public class Main {
	private static final String[] OPTIONS = { 
		"Read Shader Files", "Copy Shader Color (RGB)", "Swap Shader Colors (RGB)"
	};
	...
	public static void read(Shader[] shaders) throws IOException {
		for (Shader sh : shaders) {
			...
			if (shaderNum == 3 || shaderNum == 5)
				continue; // 03_.dat is NOT a skill shader file, but it does control them
			if (skipOdd == (shaderNum % 2 == 0)) {
				System.out.println("* " + sh.getFileName());
				if (shaderType == 0)
					sh.printRgbDataFromDat(sh.getRgbDataFromDat());
				else if (shaderType == 1)
					sh.printRgbDataFromV00(sh.getRgbDataFromV00());
	...
	public static void write(Shader[] shaders, int inColorIdx, int outColorIdx, boolean copy) throws IOException {
		...
		for (Shader sh : shaders) {
			...
			if (shaderType == 0) {
				...
				if (autoSkipResult) {
					if (!copy) textIdx = 1;
					System.out.println(text[textIdx] + " " + COLORS[outColorIdx] + " values with " + COLORS[inColorIdx]
							+ " for " + sh.getFileName() + "...");
			...
			if (args.length > 0 && args[0].equals("-c")) {
				...
				while (shaders == null) {
					...
					if (tmp.isFile()) {
						...
						File[] tmpFiles = tmp.listFiles((dir, name) -> name.toLowerCase().endsWith(".dat")
								|| name.toLowerCase().contains(".v00"));
				...
				while (option == -1) {
					...
					String input = sc.nextLine();
					if (input.matches("[0-" + (OPTIONS.length - 1) + "]"))
