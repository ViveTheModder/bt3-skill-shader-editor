...
public class Shader {
	...
	public int[] getRgbDataFromV00() throws IOException {
		...
		bin.seek(pos);
		// each color has 64 bytes of data, but only 4 are relevant (RGBA)
		int[] data = new int[4 * ((fileSize - pos) / 64)];
		int dataCnt = 0;
		while (pos < fileSize) {
			pos += 48;
			bin.seek(pos);
	...
	public void writeRgbDataToV00(int[] rgbData) throws IOException {
		...
		while (pos < fileSize) {
			...
			bin.seek(pos);
			for (int i = 0; i < 4; i++) {
				if (dataCnt < rgbData.length) bin.writeByte(rgbData[dataCnt]);
				dataCnt++;
			}
			pos += 16;
		}
